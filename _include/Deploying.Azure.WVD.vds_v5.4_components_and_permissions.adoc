...
There are some Linux thin-client vendors who have published endpoint client for WVD. These are listed at https://docs.microsoft.com/en-us/azure/virtual-desktop/linux-overview
...
** *HTML5 Portal Update* - VDS is upgrading the HTML5 service to deliver a more streamlined deployment. This will remove the need for a concurrent user limit with HTML5 connections. The new service, installed as Myrtille, is built on Microsoft's .NET framework and can be disabled or removed if HTML5 based access is not desired.
