...
* AVD stability improvement when initializing AVD – if the AVD tenant name is not unique to AVD globally, CloudJumper will replace it with an updated string unique to to the Deployment/tenant.
...
* Remove the requirement for a description of a AVD host pool (remains as and optional field)
